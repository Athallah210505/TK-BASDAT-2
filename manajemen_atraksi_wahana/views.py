...
def show_atraksi_wahana(request):
    """Show the atraksi and wahana for visitor view"""
    try:
        with connection.cursor() as cursor:
            
            cursor.execute("""
                SELECT a.nama_atraksi as nama, a.lokasi, f.kapasitas_max as kapasitas, 
                       TO_CHAR(f.jadwal, 'HH24:MI') as jadwal,
                       string_agg(DISTINCT h.nama, ', ') as hewan_terlibat
                FROM sizopi.atraksi a
                JOIN sizopi.fasilitas f ON a.nama_atraksi = f.nama
            ...
            cursor.execute("""
                SELECT w.nama_wahana as nama, w.peraturan, f.kapasitas_max as kapasitas, 
                       TO_CHAR(f.jadwal, 'HH24:MI') as jadwal
                FROM sizopi.wahana w
                JOIN sizopi.fasilitas f ON w.nama_wahana = f.nama
                ORDER BY jadwal
            ...
            for wahana in wahana_list:
                if wahana.get('peraturan'):
                    peraturan_text = wahana['peraturan']
                    if '\n' in peraturan_text:
                        wahana['peraturan_list'] = [rule.strip() for rule in peraturan_text.split('\n') if rule.strip()]
                    else:
                        wahana['peraturan_list'] = [peraturan_text]
                else:
                    wahana['peraturan_list'] = []
            
      
        print(f"DEBUG - Found {len(atraksi_list)} attractions and {len(wahana_list)} wahanas")
        
        context = {
            'attractions': atraksi_list,  
            'wahanas': wahana_list,       
            'title': 'Jadwal Pertunjukan dan Wahana',
            'error': None
        }
        return render(request, 'jadwal_pertunjukan.html', context)
    except Exception as e:
        import traceback
        print(f"ERROR in show_atraksi_wahana: {str(e)}")
        print(traceback.format_exc())
